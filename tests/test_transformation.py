import numpy as np
from geometer import Point, Line, Circle, Transformation, translation, rotation, angle
...
def test_from_points():
    ...
    assert M.apply(l).contains(M @ p1)
    assert M.apply(l).contains(M @ p3)
...
def test_translation():
    ...
    c = Circle()
    assert t.apply(c) == Circle(Point(0, -1))
...
def test_rotation():
    ...
    assert np.isclose(angle(p, t.apply(p)), a)
